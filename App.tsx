...
const PublicBookingView: React.FC<PublicBookingViewProps> = ({ barberIdFromUrl }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 md:p-8 pb-32">
      {/* Toast Alert Popups under client-side operations */}
      {toast && (
        <div
          className={`fixed bottom-10 right-10 z-[100] px-6 py-4 rounded-2xl text-[10px] font-black uppercase flex items-center gap-3 animate-in slide-in-from-right duration-300 shadow-2xl ${
            toast.type === "error" ? "bg-red-600 text-white" : "bg-emerald-500 text-black"
          }`}
        >
            ...
            {step === 2 && (
              <Card className="glass-card p-6 space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-3">
                      <p className="text-[10px] font-black text-[#E1B15F] uppercase tracking-widest px-1 italic">
                        Selecione o Dia na Agenda
                      </p>
                      ...
                      <div className="bg-slate-900/60 border border-white/5 rounded-2xl p-4 space-y-4">
                        ...
                        <div className="flex items-center justify-between pb-2 border-b border-white/5">
                          <button
                            ...
                            className="p-2 text-slate-400 hover:text-[#E1B15F] hover:bg-white/5 rounded-xl disabled:opacity-20 disabled:pointer-events-none transition-all cursor-pointer"
                          >
                            <ChevronLeft size={18} />
                          </button>
                          ...
                          <button
                            ...
                            className="p-2 text-slate-400 hover:text-[#E1B15F] hover:bg-white/5 rounded-xl transition-all cursor-pointer"
                          >
                            <ChevronRight size={18} />
                          </button>
                        </div>

                        {/* Weekday Labels (Dom, Seg, Ter...) */}
                        <div className="grid grid-cols-7 gap-1 text-center">
                          {["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"].map((label) => (
                            <span key={label} className="text-[9px] font-black text-slate-500 uppercase py-1">
                              {label}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="grid grid-cols-7 gap-1.5">
                          {calendarCells.map((cell, index) => {
                            ...
                            let btnClass = "border border-white/5 bg-slate-900/40 text-slate-300 hover:border-slate-600";
                            if (isSelected) {
                              btnClass = "bg-[#E1B15F] text-slate-950 font-black scale-105 shadow-xl shadow-[#E1B15F]/20 border-none";
                            } else if (isDisabled) {
                              btnClass = "opacity-[0.12] cursor-not-allowed bg-transparent text-slate-600 border-none pointer-events-none";
                            } else if (!hasSlots) {
                              btnClass = "border border-red-500/10 bg-red-500/5 text-slate-500";
                            } else {
                              btnClass = "border border-emerald-500/10 bg-emerald-500/5 text-emerald-400 hover:border-emerald-500/30 hover:bg-emerald-500/10";
                            }
                            ...
                            return (
                              <button
                                ...
                                className={`aspect-square flex flex-col items-center justify-center rounded-xl p-1 transition-all relative cursor-pointer ${btnClass}`}
                              >
                                ...
                                {!isDisabled && (
                                  <span className={`text-[7px] font-extrabold uppercase mt-0.5 tracking-tighter ${
                                    ...
                                    {hasSlots ? `${slots.length} h` : "Lotado"}
                                  </span>
                                )}
                              </button>
                            );
                            ...
                            {availableSlots.map((time) => (
                              <button
                                ...
                                className={`py-3.5 rounded-xl text-xs font-black transition-all cursor-pointer ${
                                  bookingTime === time
                                    ? "bg-[#E1B15F] text-slate-950 shadow-xl shadow-[#E1B15F]/20 font-black scale-105"
                                    : "bg-slate-900 text-white border border-white/5 hover:border-slate-700 hover:bg-slate-900/80"
                                }`}
                              >
                                {time}
                              </button>
                ...
                {/* Resumo do compromisso para cliente autenticado/logado */}
